...
void Application::StopNotification() {
    notify_player_.Stop();
    // 音乐播放复用同一个 Notifying 状态，所以停止通知时一并停掉音乐
    music_player_.Stop();
    audio_service_.ResetDecoder();
    auto& board = Board::GetInstance();
...
bool Application::StartMusicPlayback(const std::string& audio_url, const std::string& title) {
    ...
    auto display = board.GetDisplay();
    if (display != nullptr && !title.empty()) {
        display->SetChatMessage("assistant", title.c_str());
    }

    const bool started = music_player_.Start(audio_url, title, [this](bool success) {
        // 回调在播放任务里执行，切回主任务处理状态
    ...
    ESP_LOGI(TAG, "Music playback started: %s", title.empty() ? audio_url.c_str() : title.c_str());
    return true;
}
...
void Application::HandleMusicFinished(bool success) {
    ...
    ESP_LOGI(TAG, "Music playback %s", success ? "completed" : "failed");
    StopNotification();
}
