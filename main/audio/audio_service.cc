...
bool AudioService::PushPcmToPlaybackQueue(std::vector<int16_t>&& pcm, bool wait) {
    ...
    /* 播放队列很浅（MAX_PLAYBACK_TASKS_IN_QUEUE），这里以固定间隔等待，
     * 同时持续检查是否被 ResetDecoder() 取消，避免任何情况下卡死。 */
    while (audio_playback_queue_.size() >= MAX_PLAYBACK_TASKS_IN_QUEUE) {
        if (!wait) {
            return false;
        }
        audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(200));
        if (service_stopped_.load() || generation != playback_generation_) {
            return false;
        }
    }
