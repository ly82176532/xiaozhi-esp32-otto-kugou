...
public:
    ...
    /**
     * 把已经解码好的 PCM（单声道、采样率等于 codec->output_sample_rate()）直接送进播放队列。
     * 供 MusicPlayer 复用设备原有的喇叭输出通道（和语音播报、通知播放同一条队列）。
     *
     * @param pcm   单声道 16bit PCM 数据
     * @param wait  队列满时是否等待（true 表示会一直等到有位置或播放被取消）
     * @return false 表示服务已停止或播放已被 ResetDecoder() 取消，调用方应当立即结束
     */
