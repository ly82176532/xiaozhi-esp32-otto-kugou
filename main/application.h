...
public:
    ...
    /**
     * 播放音乐直链（MP3）。必须在设备空闲时调用，播放期间设备进入 Notifying 状态：
     * 麦克风不上行、唤醒词仍可打断、按按键/再次唤醒会立刻停止播放。
     * @return 是否成功开始播放
     */
    bool StartMusicPlayback(const std::string& audio_url, const std::string& title);
    /** 停止音乐播放（异步，不阻塞）。 */
    void StopMusicPlayback();
    bool IsMusicPlaying() const { return music_player_.IsPlaying(); }
    /** 当前播放的歌曲名，未播放时返回空串。 */
    std::string GetMusicTitle() const { return music_player_.GetTitle(); }
